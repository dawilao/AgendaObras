            ...
            with ui.tab_panels(tabs, value=tab_info).classes('w-full').style('background: transparent;'):
                ...
                with ui.tab_panel(tab_checklist).style('max-height: 370px; overflow-y: auto;'):
                    with ui.column().classes('w-full gap-1'):
                        ...
                        # Itens montados sob demanda (lazy), como no Financeiro
                        checklist_carregado = {'ok': False}
                        container_checklist = ui.column().classes('w-full gap-1')

                        def _carregar_checklist():
                            if checklist_carregado['ok']:
                                return
                            checklist_carregado['ok'] = True
                            with container_checklist:
                                self._renderizar_itens_checklist(checklist)

                        tab_checklist.on('click', _carregar_checklist)
